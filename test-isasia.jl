...
@variable(model, -P_max_line[ii, jj] <= P_line[ii in 1:N_nodes, jj in 1:N_nodes] <= P_max_line[ii, jj]) # MW
...
println("Power Flow:")
for ii in 1:N_nodes, jj in 1:N_nodes
    if value(P_line[ii,jj]) != 0 && ii != jj
        println("  - Line from $ii to $jj: ", value(P_line[ii,jj]), " MW")
    end
end
println()
